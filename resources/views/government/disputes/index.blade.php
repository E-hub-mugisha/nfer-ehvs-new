                                ...
                                <td>
                                    <div class="employee-cell">
                                        <div class="employee-avatar">
                                            {{ strtoupper(substr($dispute->employee->full_name ?? 'U', 0, 1)) }}
                                        </div>
                                        <div>
                                            <div class="employee-name">{{ $dispute->employee->full_name ?? '—' }}</div>
                                            <div class="text-muted small">{{ $dispute->employee->national_id ?? '' }}</div>
                                        </div>
                                    </div>
                                </td>
...
<style>
    ...
    /* Header */
    .page-title    { font-size:1.6rem; font-weight:700; color:#1a2a5e; margin-bottom:.2rem; }
    .page-subtitle { color:#6c757d; font-size:.9rem; margin:0; }
    .total-badge   { background:#eef0fb; color:#1a2a5e; font-weight:600; font-size:.85rem;
                     padding:.4rem .9rem; border-radius:50px; border:1px solid #d0d5ef; }

    /* Stat Cards */
    .stat-card {
        display:block; text-decoration:none; background:#fff;
        border:1.5px solid #e8eaf0; border-radius:14px; padding:1.1rem 1rem;
        transition:all .2s ease; color:inherit; position:relative; overflow:hidden;
    }
    .stat-card::before {
        content:''; position:absolute; top:0; left:0; right:0;
        height:3px; border-radius:14px 14px 0 0;
    }
    .stat-card--all::before      { background:#1a2a5e; }
    ...
    .stat-card:hover, .stat-card.active {
        border-color:transparent; box-shadow:0 4px 20px rgba(26,42,94,.12);
        transform:translateY(-2px);
    }
    .stat-icon { font-size:1.2rem; margin-bottom:.5rem; opacity:.65; }
    .stat-card--pending .stat-icon  { color:#f59e0b; }
    .stat-card--review .stat-icon   { color:#3b82f6; }
    .stat-card--resolved .stat-icon { color:#10b981; }
    .stat-card--all .stat-icon      { color:#1a2a5e; }
    .stat-count { font-size:1.6rem; font-weight:700; color:#1a2a5e; line-height:1; margin-bottom:.25rem; }
    .stat-label { font-size:.75rem; color:#6c757d; font-weight:600;
                  text-transform:uppercase; letter-spacing:.5px; }

    /* Filter Bar */
    .filter-bar  { background:#f8f9fc; border:1px solid #e8eaf0; border-radius:12px; padding:1.1rem 1.25rem; }
    .filter-label{ font-size:.75rem; font-weight:700; text-transform:uppercase;
                   letter-spacing:.5px; color:#6c757d; margin-bottom:.35rem; display:block; }
    .btn-filter  { padding:.44rem 1rem; font-size:.875rem; }

    /* Table Card */
    .disputes-table-card { background:#fff; border:1.5px solid #e8eaf0; border-radius:14px; overflow:hidden; }
    .disputes-table      { margin:0; }
    .disputes-table thead th {
        background:#f8f9fc; font-size:.73rem; font-weight:700;
        text-transform:uppercase; letter-spacing:.5px; color:#6c757d;
        border-bottom:1.5px solid #e8eaf0; padding:.85rem 1rem;
    }
    .disputes-table tbody td {
        padding:.9rem 1rem; vertical-align:middle;
        border-bottom:1px solid #f1f3f8;
    }
    .disputes-table tbody tr:last-child td { border-bottom:none; }
    .disputes-table tbody tr:hover         { background:#fafbff; }

    /* Employee Cell */
    .employee-cell   { display:flex; align-items:center; gap:.65rem; }
    .employee-avatar {
    ...
    .employee-name { font-weight:600; color:#1a2a5e; font-size:.9rem; }

    /* Misc */
    .record-ref      { font-size:.82rem; color:#4b6cb7; font-weight:500; }
    .description-cell{ font-size:.85rem; color:#495057; max-width:280px; }

    /* Status Badges */
    .status-badge { display:inline-flex; align-items:center; gap:.3rem; font-size:.75rem;
                    font-weight:600; padding:.3rem .7rem; border-radius:50px;
    ...
    /* View Button */
    .btn-view {
        background:#eef0fb; color:#1a2a5e; border:1px solid #d0d5ef;
        font-size:.8rem; font-weight:600; border-radius:8px; transition:all .2s;
    }
    .btn-view:hover { background:#1a2a5e; color:#fff; border-color:#1a2a5e; }

    /* Empty State */
    .empty-icon  { font-size:3rem; color:#c5cae9; }
    .empty-title { color:#3d4a6b; font-weight:600; }
    .empty-text  { color:#9aa0b4; font-size:.9rem; }

    /* Pagination */
    .pagination { margin:0; gap:.2rem; }
    .page-link  { border-radius:8px !important; font-size:.82rem; padding:.3rem .65rem;
                  color:#1a2a5e; border-color:#e8eaf0; }
    .page-item.active .page-link { background-color:#1a2a5e; border-color:#1a2a5e; }
</style>
@endsection