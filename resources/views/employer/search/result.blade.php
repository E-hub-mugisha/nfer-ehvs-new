...
<div class="container-fluid px-4 py-4">
    ...
    <div class="d-flex align-items-center justify-content-between mb-4 flex-wrap gap-2">
        ...
        @elseif($activeRecord && $activeRecord->employer_id !== $currentEmployer->id)
        {{-- Employee is actively employed elsewhere --}}
        @if($pendingTransfer)
        <span class="badge bg-warning text-dark py-2 px-3">
            <i class="bi bi-hourglass-split me-1"></i> Transfer Request Pending
        </span>
        @else
        <button class="btn btn-warning" data-bs-toggle="modal" data-bs-target="#requestTransferModal">
            <i class="bi bi-arrow-left-right me-1"></i> Request Transfer
        </button>
        @endif

        @else
        {{-- No active record — free to add directly --}}
...
@if(!$alreadyLinked && $activeRecord && $activeRecord->employer_id !== $currentEmployer->id && !$pendingTransfer)
<div class="modal fade" id="requestTransferModal" tabindex="-1">
    <div class="modal-dialog modal-dialog-centered">
        <div class="modal-content border-0 shadow">
            ...
            {{-- Info banner --}}
            <div class="alert alert-warning rounded-0 mb-0 small px-4 py-2 border-0">
                <i class="bi bi-info-circle me-1"></i>
                <strong>{{ $employee->first_name }}</strong> is currently employed at
                <strong>{{ $activeRecord->employer->company_name }}</strong>.
                A transfer request will be sent to them for approval.
            </div>

            <form action="{{ route('employer.transfer.store', $employee) }}" method="POST">
                @csrf
                <div class="modal-body">
                    <div class="row g-3">
                        <div class="col-12">
                            ...
                            <input type="text" name="proposed_job_title" class="form-control"
                                   required placeholder="e.g. Senior Developer">
                        </div>
                        <div class="col-12">
                            <label class="form-label fw-semibold">Department</label>
                            <input type="text" name="proposed_department" class="form-control"
                                   placeholder="e.g. Engineering">
                        </div>
                        <div class="col-12">
                            <label class="form-label fw-semibold">
                                Proposed Start Date <span class="text-danger">*</span>
                            </label>
                        ...
                        <div class="col-12">
                            ...
                            <textarea name="reason" class="form-control" rows="3"
                                      placeholder="Optional — explain why you're requesting this transfer..."></textarea>
                        </div>
                    </div>
                </div>
                <div class="modal-footer">
                    ...
                    <button type="submit" class="btn btn-warning">
                        <i class="bi bi-send me-1"></i> Send Transfer Request
                    </button>
                </div>
            </form>
        </div>
